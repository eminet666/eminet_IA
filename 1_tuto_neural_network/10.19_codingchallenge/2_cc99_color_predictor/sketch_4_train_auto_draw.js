const nb_training = 100; // nombre d'itérations d'entraînement
let brain;
let r, g, b;
let isTraining = true;
let trainingIndex = 0;
...
function setup() {
  ...
  pickColor();
  frameRate(30); // Pour un affichage fluide
}

function draw() {
  background(255);

  if (isTraining && trainingIndex < nb_training) {
    ...
    // Affichage des matrices pendant l'entraînement
    fill(192);
    ...
    text("Train: " + trainingIndex + "/" + nb_training, 450, 370);
    ...
    text("IH weights", 50, 390);
    text("HO weights", 310, 390);
    text("Train: " + trainingIndex + "/" + nb_training, 450, 390);
    displayMatrixInCanvas(brain.weights_ih, 50, 410);
    displayMatrixInCanvas(brain.weights_ho, 310, 410); 
  }
}
