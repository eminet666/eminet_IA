...
from config import VOICES, PAUSE_DURATION, AUDIO_RATE
...
async def generate_audio_from_dialogue_async(dialogue_lines, output_file="dialogue.mp3"):
    ...
    avec edge-tts pour des voix grecques naturelles, à vitesse réduite
    """
    if not dialogue_lines:
        print("⚠️  Aucune réplique trouvée dans le dialogue")
        return None
    
    print(f"- Génération audio de {len(dialogue_lines)} répliques avec edge-tts (vitesse : {AUDIO_RATE})...")
    audio_segments = []
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        for i, (speaker, text) in enumerate(dialogue_lines):
            try:
                ...
                # Générer l'audio avec edge-tts en appliquant le taux de vitesse
                communicate = edge_tts.Communicate(text, voice, rate=AUDIO_RATE)
