...
def send_email(gmail_user, gmail_password, content, audio_file=None, pdf_file=None, title=None):
    ...
    if title:
        msg["Subject"] = f"espAgent : {title} - Dialogue espagnol quotidien"
    else:
        msg["Subject"] = "espAgent : Dialogue espagnol quotidien 🎧"

    # Titre pour l'en-tête de l'email
    email_title = f"{title}" if title else "Dialogue en espagnol (niveau A2)"
    ...
    if audio_file and os.path.exists(audio_file):
        try:
            ...
            filename = f"dialogue_espagnol_{datetime.now().strftime('%Y%m%d')}.mp3"
    ...
    if pdf_file and os.path.exists(pdf_file):
        try:
            ...
            encoders.encode_base64(part)
            filename = f"dialogue_espagnol_{datetime.now().strftime('%Y%m%d')}.pdf"
            part.add_header("Content-Disposition", f"attachment; filename= {filename}")
            msg.attach(part)
